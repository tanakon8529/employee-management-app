...
def read_position_table_by_condition(data, db_session):
    ...
    try:
        if data.id:
            ...
            payload_Position = db_session.query(Position).filter(Position.name == data.name).first()
        elif data.created_at:
            payload_Position = db_session.query(Position).filter(Position.created_at == data.created_at).all()
        elif data.updated_at:
            payload_Position = db_session.query(Position).filter(Position.updated_at == data.updated_at).all()
        else:
            payload_Position = db_session.query(Position).all()

        if payload_Position:
            result = {"detail": "Position read successfully", "data": [Position_Base(**payload_Position.__dict__) for payload_Position in payload_Position]}
        else:
